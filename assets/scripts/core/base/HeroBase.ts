...
export class HeroBase extends Component {
    ...
    @property({type: Node, displayName: "血条点"})
    public heroHpPot: Node = null as unknown as Node
    ...
    onLoad() {
        ...
        if (!this.heroBasePot) {
            ...
            console.warn(this.node.getParent()?.name + "英雄未配置原点");
        }

        if (!this.heroMainWeaponPot) {
            this.heroMainWeaponPot = this.node;
            console.warn(this.node.getParent()?.name + "英雄未配置主武器");
        }

        if (!this.heroChestPot) {
            this.heroChestPot = this.node;
            console.warn(this.node.getParent()?.name + "英雄未配置胸骨");
        }

        if (!this.heroSubWeaponPot) {
            this.heroSubWeaponPot = this.node;
            console.warn(this.node.getParent()?.name + "英雄未配置副武器");
        }

        if (!this.heroCenterPot) {
            this.heroCenterPot = this.node;
            console.warn(this.node.getParent()?.name + "英雄未配置中心点");
        }

        if (!this.heroHpPot) {
            this.heroHpPot = this.node;
            console.warn(this.node.getParent()?.name + "英雄未配置血条点");
        }
